...
Flashes Bank Board firmware (white case) with a unique Vault#-Security
WiFi AP for each station. Each board gets sensor + dashboard + UART TX.

Blue case (attacker) boards ship blank — students flash those themselves.
...
The script will:
  ...
  2. Ask for the vault number (1-20)
  3. Compile with the correct vault number and flash
  4. Verify the flash by reading the startup banner
...
FIRMWARE = {
    "bank": os.path.join(SCRIPT_DIR, "firmware", "gateway_test", "gateway_test.ino"),
}
...
def print_banner():
    ...
    print("  Islanders Cybersecurity Club")
    print("=" * 50)
...
def main():
    ...
    while True:
        print()
        print("-" * 50)
        print("  Plug in a board and press Enter")
        ...
        print(f"  (sensor + dashboard + UART TX)")
